...
- **[infra-only-pipeline.yml](infra-only-pipeline.yml):** Creates all of the Azure resources by deploying the [main.bicep](../../infra/bicep/main.bicep) template.
- **[aca-build-deploy-pipeline.yml](aca-build-deploy-pipeline.yml):** Pipeline to build the application source code, store the resulting images in the Azure Container Registry, then deploy to the Azure Container Apps environments.
